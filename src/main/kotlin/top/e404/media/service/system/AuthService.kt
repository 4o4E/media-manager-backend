...
class AuthServiceImpl : AuthService {
    ...
    override fun login(dto: LoginDto): LoginVo {
        ...
        // 检查密码
        val result = BCrypt.verifyer().verify(dto.password.toCharArray(), base64Service.decode(user.password!!))
        // 密码错误
    ...
    override fun resetPassword(dto: ResetPasswordDto) {
        ...
        userService.update(
            LambdaUpdateWrapper<UserDo>()
                .set(
                    UserDo::password,
                    BCrypt.withDefaults().hashToChar(12, base64Service.decode(password).toCharArray())
                )
        )
    }
}